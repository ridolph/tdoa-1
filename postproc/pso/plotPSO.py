...
pos2=np.loadtxt('plotPSO\\3pso.txt')

cnt=0
for i in range(len(pos2)):
    if(pos2[i,0]==200):
        cnt=cnt+1
RMSE=np.sqrt(np.mean((pos2[:,1]-pos2[:,4])**2+(pos2[:,2]-pos2[:,5])**2+(pos2[:,3]-pos2[:,6])**2))
MEAN=np.mean(np.sqrt((pos2[:,1]-pos2[:,4])**2+(pos2[:,2]-pos2[:,5])**2+(pos2[:,3]-pos2[:,6])**2))
